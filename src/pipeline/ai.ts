...
export async function enrichWithAI(jobs: ScoredJob[], max = 10): Promise<ScoredJob[]> {
    ...
    const enriched = await Promise.all(toEnrich.map(async (job) => enrichOne(job, key)));
...
export async function enrichWithAIHeavy(jobs: ScoredJob[], max = 150): Promise<ScoredJob[]> {
    ...
    console.log(`[ai-heavy] done ${enriched.length}, fallback ${toEnrich.length - enriched.filter((j) => j.aiExplanation).length} without AI`);
    return [...enriched, ...rest].sort((a, b) => b.score - a.score);
}
async function enrichOne(job: ScoredJob, key: string): Promise<ScoredJob> {
    try {
        const prompt = `You are a job matching assistant for a Full Stack Developer (React, TypeScript, Node.js, PostgreSQL, Tailwind, Hono, Drizzle) — based in India, open to remote India.
...
Only JSON, no markdown.`;
        ...
        return {
            ...
            score: typeof parsed.adjustedScore === "number"
                ? Math.max(0, Math.min(100, Math.round(job.score * 0.6 + parsed.adjustedScore * 0.4)))
                : job.score,
        } as ScoredJob;
    }
    catch (e) {
