...
export async function loadHealth(path = "data/ats-health.json"): Promise<HealthStore> {
    try {
...
export async function saveHealth(store: HealthStore, path = "data/ats-health.json"): Promise<void> {
...
export function buildHealthMarkdown(store: HealthStore): string {
    ...
    if (disabled.length) {
        ...
        for (const h of disabled)
            md += `- \`${h.board}\` — fails ${h.consecutiveFails}, last fail ${h.lastFail?.slice(0, 10)} → until ${h.disabledUntil?.slice(0, 10)}\n`;
        md += "\n";
    }
    if (failing.length) {
        md += `## 🟡 Failing\n`;
        for (const h of failing)
            md += `- \`${h.board}\` — ${h.consecutiveFails}x fails, last ${h.lastFail?.slice(0, 10)}, success ${h.success}\n`;
        md += "\n";
    }
    if (stale.length) {
        md += `## ⚪ Stale (0 jobs last fetch)\n`;
        for (const h of stale.slice(0, 10))
            md += `- \`${h.board}\` — avg ${h.avgJobs ?? 0}, last ${h.lastJobCount ?? 0} jobs\n`;
        if (stale.length > 10)
            md += `- ... and ${stale.length - 10} more\n`;
        md += "\n";
    }
